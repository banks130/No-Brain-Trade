...
# Import config – make sure PRIVATE_KEY is here
from config import (
...
class SignalBot:
    def __init__(self, trader=None, mm=None):
        ...
        self.user_wallets: Dict[int, Keypair] = {}          # user_id -> Keypair
        self.user_traders: Dict[int, UserTrader] = {}       # user_id -> UserTrader
        self.mm_requests: List[dict] = []                   # pending MM purchase notifications
        self.trader = trader    # original (unused now, but kept for compatibility)
        self.mm = mm            # original MarketMaker instance
    ...
    async def cmd_mm_purchase(self, update, context):
        ...
        # Admin wallet must be available (from PRIVATE_KEY in config)
        if not PRIVATE_KEY:
    ...
    def register_handlers(self, application: Application):
        application.add_handler(CommandHandler("start", self.cmd_start))           # ✅ FIXED
        app = application
        # Wallet
