...
from solders.transaction import Transaction         # ✅ correct import
...
class SignalBot:
    ...
    async def start_cmd(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_id == TELEGRAM_ADMIN_ID:
            msg = ("Admin commands available.\n"
                   "Public: /spikes, /mm_status, /trades, /help\n"
                   "Admin: /balance, /send, /withdraw_all, /mm_start, /mm_stop, /emergency_kill, /toggle_auto_buy")
        else:
    ...
    async def balance_cmd(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await update.message.reply_text(f"💰 Balance: {balance_lamports/1e9:.4f} SOL")
        except Exception as e:
    ...
    async def send_cmd(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if len(context.args) != 2:
            ...
            return
        to_address, amount_sol = context.args[0], float(context.args[1])
        if amount_sol <= 0:
        ...
        try:
            ...
            ix = transfer(TransferParams(from_pubkey=self.keypair.pubkey(), to_pubkey=to_pubkey, lamports=int(amount_sol*1e9)))
    ...
    async def withdraw_all_cmd(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            to_pubkey = Pubkey.from_string(to_address)
            pubkey = self.keypair.pubkey()
            balance_lamports = (await self.rpc_client.get_balance(pubkey, commitment=Confirmed))['result']['value']
            if balance_lamports <= 5000:
                await update.message.reply_text("Balance too low.")
                return
            if DRY_RUN:
                await update.message.reply_text(f"DRY RUN: Would withdraw {(balance_lamports-5000)/1e9:.4f} SOL to {to_address}")
                return
            amount_lamports = balance_lamports - 5000
            blockhash = (await self.rpc_client.get_latest_blockhash(commitment=Confirmed))['result']['value']['blockhash']
            ix = transfer(TransferParams(from_pubkey=pubkey, to_pubkey=to_pubkey, lamports=amount_lamports))
            tx = Transaction().add(ix)
            tx.recent_blockhash = blockhash
            tx.sign(self.keypair)
            result = await self.rpc_client.send_transaction(tx, TxOpts(skip_preflight=False, preflight_commitment=Confirmed))
